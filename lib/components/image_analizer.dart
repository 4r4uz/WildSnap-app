...
class ImageClassifier {
  ...
  Future<void> _loadModels() async {
    ...
    try {
      ...
      _detectionInterpreter = await Interpreter.fromAsset('assets/deteccion.tflite', options: options);
      ...
      _classificationInterpreter = await Interpreter.fromAsset('assets/clasificacion.tflite', options: options);
      ...
      try {
        _detectionInterpreter = await Interpreter.fromAsset('assets/deteccion.tflite', options: minimalOptions);
        _detectionInterpreter.allocateTensors();
        debugPrint('✅ Detection model loaded with minimal options');

        _classificationInterpreter = await Interpreter.fromAsset('assets/clasificacion.tflite', options: minimalOptions);
        _classificationInterpreter.allocateTensors();
  ...
  Future<Map<String, dynamic>> _runClassification(img.Image crop) async {
    ...
    // Preprocesamiento EfficientNet (normalización ImageNet)
    for (int i = 0; i < resizedBytes.length; i += 3) {
      input[i] = (resizedBytes[i] - 127.5) / 127.5;     // R
      input[i + 1] = (resizedBytes[i + 1] - 127.5) / 127.5; // G
      input[i + 2] = (resizedBytes[i + 2] - 127.5) / 127.5; // B
    }

    final inputTensor = input.reshape([1, _classificationInputSize, _classificationInputSize, 3]);

    // Output para clasificación
    final output = List.filled(_classificationLabels.length, 0.0);
    ...
    return {
      'label': maxIndex >= 0 ? _classificationLabels[maxIndex] : 'unknown',
      'confidence': maxProb,
      'classIndex': maxIndex,
    };
  }
